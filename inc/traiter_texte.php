...
function _traiter_code($regs) {
	...
	$le_hash = traiter_echap_code_dist([null,null,null,$code]);
...
function _creer_lien_riche($lien) {
	...
	if (
		...
		foreach ($b as $img) {
			if (
				extraire_attribut($img, 'id') == 'image-view'
	...
	if (preg_match(',\.(png|gif|jpe?g|svg),i', $lien)) {
		...
		$image = afficher_miniature($lien, 600, 700);
...
function _texte_inserer_embed($regs) {
	...
	if (
		...
		if (
			$c = extraire_attribut($embed, 'class')
			and preg_match(',\boembed-source-\w+\b,', $c, $r)
		) {
			$class .= ' ' . $r[0];
		}

		$lienclass = inserer_attribut(
			$lien,
			'class',
