...
function notifier_partage($id_auteur_partage, $id_me) {
	...
	$annonce = _T('seenthis:notif_partage_texte', [
		'nom_dest' => $nom_dest,
		'nom_aut_partage' => $nom_aut_partage,
		'login_aut_partage' => $login_aut_partage,
		'seenthis' => $seenthis,
...
function seenthis_message_footer($lang, $seenthis) {
	return _T('seenthis:notif_message_footer', [
		'seenthis' => $seenthis,
		'url' =>  _HTTPS . '://' . _HOST,
		'spip_lang' => $lang
	]);
	...
	if ($row_aut = sql_fetch($query_aut)) {
		...
		if ($row_dest = sql_fetch($query_dest)) {
			...
			if (strlen(trim($email_dest)) > 3) {
				...
				$annonce = _T('seenthis:notif_suivremoi_texte', [
					'nom_dest' => $nom_dest,
					'nom_aut' => $nom_aut,
					'login_aut' => $login_aut,
					'seenthis' => $seenthis,
...
function notifier_me($id_me, $id_parent) {
	...
	$id_dest = pipeline(
		'seenthis_notifierme_destinataires',
		[
			'args' => ['id_me' => $id_me,'id_parent' => $id_parent],
			'data' => $id_dest
		]
	);
	...
	if (isset($id_dest) and count($id_dest) > 0) {
		...
		while ($row_dest = sql_fetch($query_dest)) {
			...
			if (strlen(trim($email_dest)) > 3) {
				...
				if ($id_parent == 0) {
					$annonce = _T('seenthis:notif_me_nouveau', ['nom_auteur' => $nom_auteur, 'spip_lang' => $lang]);
				} else {
					if ($nom_auteur == $nom_auteur_init) {
						$annonce = _T('seenthis:notif_me_reponse', ['nom_auteur' => $nom_auteur, 'spip_lang' => $lang]);
					} else {
						$annonce = _T('seenthis:notif_me_reponse_a', ['nom_auteur' => $nom_auteur, 'nom_auteur_init' => $nom_auteur_init, 'spip_lang' => $lang]);
					}
				}
