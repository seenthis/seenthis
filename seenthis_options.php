...
function identifier_url($url, $id_parent) {
	...
	if ($row = sql_fetch($query)) {
		...
		if ($id_syndic > 0) {
			job_queue_add('recuperer_contenu_site', 'récupérer_contenu_site ' . $url, [$id_syndic, "$url"]);
		}
...
function hierarchier_url($id_syndic) {
	...
	if ($row = sql_fetch($query)) {
		...
		if ($url_site != $chemin_complet) {
			identifier_url("$url_site", $id_parent);
		}
	}
}
...
function allonger_url($url) {
	...
	if (!preg_match('/(seen\.li|youtu\.be|t\.co|reut\.rs|nyti\.ms|fb\.me|bit\.ly|goo\.gl|2tu\.us|icio\.us|tinyurl\.com|tr\.im|ur1\.ca|a\.pwal\.fr|j\.mp|is\.gd|a\.gd|ow\.ly|spedr\.com|shar\.es|twurl\.nl|shr\.im|u\.nu|ff\.im|bt\.io|minu\.me|zi\.pe)/', $url)) {
		return $url;
	}
	...
	if (preg_match('#Location: (.*)#', $a, $r)) {
		$l = trim($r[1]);
		if (!preg_match(',^(http|ftp)s?\:,', $l)) {
			$l = curl_getinfo($ch, CURLINFO_EFFECTIVE_URL);
		}
	}
	return $l;
}
		...
		if ($contenu) {
			...
			if (!is_utf8($html)) {
				...
				foreach (extraire_balises($head, 'meta') as $meta) {
					if (
						$equiv = extraire_attribut($meta, 'http-equiv')
...
function fichier_bandeau($id_auteur, $avec_date = true) {
	...
	$fichier = false;
	if (file_exists("$racine.jpg")) {
		$fichier = "$racine.jpg";
	} elseif (file_exists("$racine.png")) {
		$fichier = "$racine.png";
	} elseif (file_exists("$racine.gif")) {
		$fichier = "$racine.gif";
	}
	if ($fichier && $avec_date) {
		$fichier = $fichier . '?' . filemtime($fichier);
	}
	return $fichier;
}
	...
	if ($troll_forcer > 0) {
		...
		while ($row = sql_fetch($query)) {
			...
			if ($troll == 0) {
				$troll = $row['troll'];
				if ($reseau) {
					$troll = calculer_troll($id_follow, false);
				}
			}
...
function afficher_troll($id_auteur) {
	...
	if (isset($val_troll["$id_auteur"])) {
		return $val_troll["$id_auteur"];
	} else {
...
function nom_auteur($id_auteur) {
	if ($GLOBALS['nom_auteur']["$id_auteur"]) {
...
function instance_me($id_auteur = 0, $texte_message = '', $id_me = 0, $id_parent = 0, $time = 'NOW()', $uuid = null, $viarss = 0) {
	...
	if ($id_me == 0) {
		...
		if (!autoriser('modifier', 'me', $id_me)) {
			die('Forbidden');
		}
	...
	return ['id_me' => $id_me, 'id_parent' => $id_parent, 'maj' => $maj];
}
		...
		foreach ($regs[0] as $k => $url) {
			...
			if (!$deja_vu['url'][$url]) {
				...
				if ($row = sql_fetch($query)) {
					...
					$query_total = sql_select(
						'spip_me.id_me AS id_me_supp, spip_me.id_parent AS id_parent_supp, spip_me.id_auteur AS id_auteur_supp',
...
function urlencode_1738_plus($url) {
	...
	for ($i = 0; $i < $l; $i++) {
		$u = ord($a = $url[$i]);
		if ($u <= 0x20 or $u >= 0x7F or in_array($a, ["'",'"'])) {
			$a = rawurlencode($a);
		}
...
function seenthis_affichage_final($t) {
	...
	if (
		...
		if ($m = @array_diff(array_unique($m[2]), [$id_auteur])) {
			include_spip('base/abstract_sql');
